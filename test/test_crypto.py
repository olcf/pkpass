...
class TestBasicFunction(unittest.TestCase):
  ...
  def test_encrypt_string(self):
    results = []
    for key, identity in self.identities.iddb.iteritems():
  ...
  # Encrypt/Decrypt strings for all test identities and make sure we get back what we put in
    for key, identity in self.identities.iddb.iteritems():
      (ciphertext, derived_key) = crypto.pk_encrypt_string(self.plaintext, identity)
      plaintext = crypto.pk_decrypt_string(ciphertext, derived_key, identity, None)
  ...
  def test_verify_string(self):
    results = []
    for key, identity in self.identities.iddb.iteritems():
      results.append(crypto.pk_sign_string(self.plaintext, identity, None))
    self.assertTrue( results[0] != results[1] )
