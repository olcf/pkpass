...
class PasswordEntry(object):
  ...
  def add_recipients(self, secret = None, distributor = None, recipients = [], identitydb = None, encryption_algorithm = 'rsautl', passphrase = None):
    ##############################################################################
    """ Add recipients to the recipient list of this password object           """
    ##############################################################################
    for recipient in recipients:
      if( encryption_algorithm == 'rsautl' ):
        (encrypted_secret, encrypted_derived_key) = crypto.pk_encrypt_string(secret, identitydb.iddb[recipient])
  ...
  def decrypt_entry(self, identity = None, passphrase = None ):
    ...
    try:
      ...
      raise NotARecipientError("Identity '%s' is not on the recipient list for password '%s" % (identity['uid'], self.metadata['name']))

    try:
      return crypto.pk_decrypt_string( recipient_entry['encrypted_secret'],
                                       recipient_entry['derived_key'],
                                       identity,
                                       passphrase )
    except DecryptionError:
      raise DecryptionError("Error decrypting password named '%s'.  Perhaps a bad pin/passphrase?" % self.metadata['name'])
  ...
  def verify_entry(self, uid = None, identitydb = None):
    ...
    message = self._create_signable_string(recipient_entry)
    sigOK = crypto.pk_verify_signature( message, signature, identitydb[distributor] )
