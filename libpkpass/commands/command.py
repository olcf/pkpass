...
class Command(object):
  ...
  def _run_command_setup(self, parsedargs):
    ...
    for key, value in cli_args.iteritems():
      if value != None or key not in self.args:
        self.args[key] = value
    ...
    self.identities.load_certs_from_directory(self.args['certpath'], self.args['cabundle'])
  ...
  def _validate_identities(self):
    for recipient in self.recipient_list:
      if recipient not in self.identities.iddb.keys():
        raise CliArgumentError("Error: Recipient '%s' is not in the recipient database" % recipient)

    if self.args['identity'] not in self.identities.iddb.keys():
      raise CliArgumentError("Error: Your user '%s' is not in the recipient database" % self.args['identity'])
