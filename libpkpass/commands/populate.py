...
class Populate(Show):
    ...
    name = "populate"
    description = (
        "Populate external resource with password, currently supports: puppet_eyaml"
    )
    selected_args = [
        "pwname",
        "cabundle",
    ...
    def _run_command_execution(self):
        ...
        if pop_type == "kubernetes":
            ...
            for password in self.args["populate"][pop_type]["passwords"].keys():
                ...
                self._decrypt_wrapper(self.args["pwstore"], pwentry, password, pop_type)
            ...
            yield from self._decrypt_wrapper(
                self.args["pwstore"], password, self.args["pwname"], pop_type
            )
            ...
            if not match_iter:
                ...
                for match_values in match_iter:
                    ...
                    new_pw = new_pw.replace(
                        f"%{{{match_value}}}", plaintext[match_value]
                    )
    ...
    def _handle_puppet(self, plaintext_pw, pwname):
        ...
        if self.args["value"]:
            ...
            for hiera_file, names in self.args["populate"]["puppet_eyaml"]["passwords"][
                pwname
            ].items():
                with open(
                    path.join(directory, hiera_file), "r", encoding="ASCII"
                ) as data_file:
                ...
                with open(
                    path.join(directory, hiera_file), "w", encoding="ASCII"
                ) as data_file:
